...
def required_input(input_text, input_func=input):
    ...
    first_run = True
    while response == '':
        response = input_func('{0}{1}'.format('✘ ' if not first_run else '', input_text))
        first_run = False
...
def generate_secret_key(key_length, display_key):
    key_array = [random.SystemRandom().choice(string.printable[:-6]) for i in range(key_length)]
...
if __name__ == '__main__':
    ...
    print('---')
    print('The "user-core" service uses PostgreSQL as the database backend. Each machine (dev or production) may have differing database names and credentials for this service. Please provide the following information to connect your local copy of this service to the PostgreSQL server you have set up for this service.')
    print('')
    ...
    db_host = input('Database host (leave blank for the default, \'localhost\'): ') or 'localhost'
    db_port = input('Database port (leave blank for the default, \'5432\'): ') or '5432'
    print('Got it! Thanks!')
    ...
    secret_key = generate_secret_key(key_length, yesno_input('Display key [Y/N]? '))
    ...
    if os.name == 'posix':  # Yeah, we're in UNIX boiz.
        ...
        batch_file.write('# This file is auto-generated. Do not modify unless you know what you are doing.\n')
        batch_file.write('\n')
        batch_file.write('echo " [*] Exporting variables.\n')
        batch_file.write('export USERCORE_DB_NAME={0}\n'.format(db_name))
        batch_file.write('export USERCORE_DB_USERNAME={0}\n'.format(db_username))
        batch_file.write('export USERCORE_DB_PASSWORD={0}\n'.format(db_password))
        batch_file.write('export USERCORE_DB_HOST={0}\n'.format(db_host))
        batch_file.write('export USERCORE_DB_PORT={0}\n'.format(db_port))
        batch_file.write('export USERCORE_SECRET_KEY={0}\n'.format(secret_key))
        batch_file.close()
